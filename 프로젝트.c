...
int *check_mcb(int s_number[], int number[])                                 //number 배열에는 문제 번호들이 들어있음
{
	int answer[30] = { 1, 2, 1, 2, 1, 1, 2, 2, 2, 1, 1, 1, 1, 2, 2, 2, 1, 2, 1, 1, 2, 1, 1, 2, 1, 2, 1, 2, 1, 2 };
	int wrongq[15] = {0};
	int i, k = 0;
	int x;
	for (i = 0; i<15; i++)
...
void odap_mcb(int* wrongq, char mcb_q[][100])
{
	...
	{
		printf("오답노트를 출력하려면 0을 입력하세요(끝내려면 아무거나 입력) : ");
		scanf("%d", &dd);
		if (dd == 0)
		{
			printf("===================================\n");
		...
		if (dd == 0)
		{
			mcbtext(mcb_q, wrongq, hs);
		}
	}	
}
void mcbtext(char q[][100], int *wrongq, char hs[][100])
{
	FILE* fw = fopen("맞춤법 오답노트.txt", "w");
	int n;
	for (int i = 0; i < 10; i++)
	{
		n = wrongq[i];
...
void easyword_hs(int wrongq[])
{
	...
	if (n == 0)
	{
		easywordtext(wordhs, wrongq);
	}
}

void easywordtext(char ary[][100], int* wrongq)
{
	FILE* fw = fopen("어휘 정답(기본).txt", "w");
	int n;
	for (int i = 0; i < 10; i++)
	{
		n = wrongq[i];
